...
def inDelphi_logit(ref: str, mh_gt_pos: List, mh_del_len: List, mh_mh_len: List, mh_weight: torch.Tensor, mhless_weight: torch.Tensor, pre_insert_probability: float, pre_insert_1bp: np.array):
    pre_template_insert_probability = pre_insert_1bp[base_map[ref[args.cut1 - 1]]] * pre_insert_probability
    if ref[args.cut1] != ref[args.cut1 - 1]:
        pre_template_insert_probability += pre_insert_1bp[base_map[ref[args.cut1]]] * pre_insert_probability
    pre_delete_probability = (
        F.normalize(
            torch.cat([
                mh_weight,
                mhless_weight
    ...
    mh_shift = torch.cat([
        torch.arange(mh_len_p1) for mh_len_p1 in mh_mh_len_p1
    ])
    ref2start -= mh_shift
    ref1end -= mh_shift

    pre_probability = torch.zeros(args.ref2len + 1, args.ref1len + 1)
    pre_probability[ref2start, ref1end] += (mh_weight / mh_mh_len_p1).repeat_interleave(mh_mh_len_p1)
    pre_probability[inDelphi_ref2start_mhless, inDelphi_ref1end_mhless] += (mhless_weight / inDelphi_del_len_p1).repeat_interleave(inDelphi_del_len_p1)
    pre_probability[args.cut2 - 1, args.cut1] = pre_insert_1bp[base_map[ref[args.cut1 - 1]]] * pre_insert_probability
    if ref[args.cut1] != ref[args.cut1 - 1]:
        pre_probability[args.cut2, args.cut1 + 1] = pre_insert_1bp[base_map[ref[args.cut1]]] * pre_insert_probability
    return pre_probability.log().clamp_min(zero_logit)

inDelphi_del_len_p1 = torch.arange(2, args.DELLEN_LIMIT + 1)
inDelphi_ref2start_mhless = torch.cat([
    torch.arange(del_len_p1) for del_len_p1 in inDelphi_del_len_p1
]) + args.cut2
...
def inDelphi_benchmark(data_name: str, black_list: List):
    ...
    with open(args.output_dir / inDelphiConfig.model_type / f"{data_name}_{inDelphiConfig.model_type}" / "insertion_model.pkl", "rb") as fd:
    ...
    test_dataloader = DataLoader(
        ...
        collate_fn=lambda examples: data_collector(examples, args.DELLEN_LIMIT, outputs_test + ["genotype_count"])
    )

    likelihood, genotype_pearson, total_del_len_pearson, mhless_pearson, mhless_count_acc, total_del_len_count_acc = [], [], [], [], torch.zeros(args.DELLEN_LIMIT - 1, dtype=torch.int64), torch.zeros(args.DELLEN_LIMIT - 1, dtype=torch.int64)
    for examples, batch, ob_examples in tqdm(
        ...
        total_del_len_count_acc += batch['total_del_len_count'].to(torch.int64).sum(dim=0)

        output = pipe(batch)
        max_mh_len = batch['genotype_count'].shape[1] - output['mhless_weight'].shape[0]
        mh_weight = torch.stack([torch.cat([mw, torch.zeros(max_mh_len - len(mw), device=mw.device)]) for mw in output['mh_weight']])
        ...
        genotype_pearson.extend(
            (
                ...
                F.normalize(batch['genotype_count'], p=2.0, dim=1).to(mh_weight.device)
            ).sum(dim=1).tolist()
        )
        total_del_len_pearson.extend(
            (
                F.normalize(output['total_del_len_weight'], p=2.0, dim=1) *
                F.normalize(batch['total_del_len_count'], p=2.0, dim=1).to(output['total_del_len_weight'].device)
            ).sum(dim=1).tolist()
        )
        mhless_pearson.extend(
            (
                F.normalize(output['mhless_weight'], p=2.0, dim=0) *
                F.normalize(mhless_count.to(torch.float32), p=2.0, dim=1).to(output['mhless_weight'].device)
            ).sum(dim=1).tolist()
        )

        for i in range(len(examples['ref'])):
            observation = torch.zeros(args.ref2len + 1, args.ref1len + 1)
            observation[ob_examples['ob_ref2'][i], ob_examples['ob_ref1'][i]] = torch.tensor(ob_examples['ob_val'][i], dtype=observation.dtype)
            ...
            likelihood.append(
                black_list_likelihood(observation, logit, black_list)
            )

    return likelihood, genotype_pearson, total_del_len_pearson, mhless_pearson, mhless_count_acc, total_del_len_count_acc, output['mhless_weight'].cpu()

@torch.no_grad()
def inDelphi_original_benchmark(data_name: str, black_list: List, celltype: str): # Supported cell types are ['mESC', 'U2OS', 'HEK293', 'HCT116', 'K562']
    ...
    inDelphi_original_proc = Popen(f"./benchmark_inDelphi_original.py {celltype}", shell=True, executable="/bin/bash", stdin=PIPE)
    for i in tqdm(
        range(len(ds_ob)),
        desc=f"inDelphi original {celltype}, logit"
    ):
        ref = (
    ...
    pred_df_all = pd.read_csv(
        ...
        names=["Category", "Genotype position", "Inserted Bases", "Length", "Predicted frequency"],
        header=None
    )
    ...
    for examples, batch, ob_examples in tqdm(
        ...
        for i in range(len(ob_examples['ref1'])):
            ref = ob_examples['ref1'][i][:args.cut1] + ob_examples['ref2'][i][args.cut2:]
            ...
            logit = inDelphi_logit(ref, mh_gt_pos, mh_del_len, mh_mh_len, mh_weight, mhless_weight, pre_insert_probability, pre_insert_1bp)
...
def FOREcasT_benchmark(data_name: str, black_list: List):
    ...
    for batch, ob_examples in tqdm(
        ...
        for i in range(len(ob_examples['ref1'])):
            ...
            ref1, ref2, cut1, cut2 = ob_examples['ref1'][i], ob_examples['ref2'][i], ob_examples['cut1'][i], ob_examples['cut2'][i]
            lefts, rights = [], []
            for left, right, ins_seq in zip(output['left'], output['right'], output['ins_seq']):
                templated_left, templated_right = detect_templated_insert(left + cut1, right + cut2, ins_seq, ref1, ref2)
                lefts.append(templated_left)
                rights.append(templated_right)
            
            pre_probability = torch.sparse_coo_tensor([rights, lefts], output['proba'][i].cpu(), (args.ref2len + 1, args.ref1len + 1)).to_dense()
    ...
    for batch, ob_examples in tqdm(
        ...
        for i in range(len(ob_examples['ref1'])):
            ...
            for ins_seq in output['ins_base'][:-1]:
                templated_left, templated_right = detect_templated_insert(cut1, cut2, ins_seq, ref1, ref2)
            ...
            pre_probability = torch.sparse_coo_tensor([drights + irights, dlefts + ilefts], all_proba, (args.ref2len + 1, args.ref1len + 1)).to_dense().cpu()
...
def CRISPR_transformer_benchmark(data_name: str, black_list: List):
    from ..CRISPR_transformer.model import CRISPRTransformerModel, CRISPRTransformerConfig
...
def all_benchmark(data_name: str, black_list: List):
    ...
    # inDelphi
    ####################
    likelihood, pearson_genotype, pearson_total, pearson_mhless, accum['mhless_count'], accum['total_count'], accum['inDelphi_mhless_weight'] = inDelphi_benchmark(data_name, black_list)
    benchmark_df_rows.extend(
        [
            {
                'value': likelihood,
            ...
            {
                'value': [tensor_correlation(accum['inDelphi_mhless_weight'], accum['mhless_count'])],
                'stat': 'pearson',
                'model': 'inDelphi',
                'pearson_type': 'mhless_acc'
            },
            {
                'value': [tensor_correlation(accum['inDelphi_mhless_weight'], accum['total_count'])],
                'stat': 'pearson',
                'model': 'inDelphi',
    ...
    for celltype in ['mESC']: # ['mESC', 'U2OS', 'HEK293', 'HCT116', 'K562']
        likelihood, pearson_genotype, pearson_total, pearson_mhless, accum[f'original_mhless_weight_{celltype}'] = inDelphi_original_benchmark(data_name, black_list, celltype)
        benchmark_df_rows.extend(
            [
                {
                    'value': likelihood,
                ...
                {
                    'value': [tensor_correlation(accum[f'original_mhless_weight_{celltype}'], accum['mhless_count'])],
                    'stat': 'pearson',
                    'model': f'original_{celltype}',
                    'pearson_type': 'mhless_acc'
                },
                {
                    'value': [tensor_correlation(accum[f'original_mhless_weight_{celltype}'], accum['total_count'])],
                    'stat': 'pearson',
                    'model': f'original_{celltype}',
                    'pearson_type': 'total_acc'
                }
    ...
    # save
    ####################
    pd.DataFrame(benchmark_df_rows).explode('value', ignore_index=True).to_csv(f'AI_models/benchmark/{data_name}_benchmark.csv', index=False)
    pd.DataFrame(accum).to_csv(f'AI_models/benchmark/{data_name}_accum.csv', index=False)
