...
def black_list_logit(logit, black_list):
    proba = F.softmax(logit.flatten(), dim=0).view(logit.shape)
    proba[*black_list] = 0
    return F.normalize(
        proba.flatten(),
        p=1,
        dim=0
    ).log().clamp_min(zero_logit).view(logit.shape)
...
@torch.no_grad()
def inDelphi_benchmark(black_list: List):
    ...
    for examples, batch, ob_examples in tqdm(
        ...
        for i in range(len(examples['ref'])):
            ...
            logit = black_list_logit(logit, black_list)
...
@torch.no_grad()
def inDelphi_original_benchmark(black_list: List, celltype: str): # Supported cell types are ['mESC', 'U2OS', 'HEK293', 'HCT116', 'K562']
...
def Lindel_benchmark(black_list: List):
