...
class CRISPRDiffuserPipeline(DiffusionPipeline):
    ...
    def __call__(self, batch, batch_size=1, record_path=False):
        ...
        t = self.scheduler.step_to_time(
            torch.tensor([self.scheduler.config.num_train_timesteps])
        )
        if record_path:
        ...
        for timestep in tqdm(self.scheduler.timesteps):
            ...
            x1t, x2t, t = self.scheduler.step(
                p_theta_0_logit,
                x1t,
                x2t,
                t,
                self.stationary_sampler1,
                self.stationary_sampler2,
            )
            if record_path:
                x1ts.append(x1t)
                x2ts.append(x2t)
