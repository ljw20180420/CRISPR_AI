...
\begin{eqnarray}\label{EqDivParaHatR}
  &&g^{(d)}_t\qty(y^{(d)}\middle|x^{(1:D)})\coloneq\sum_{x^{(d)}_0}\frac{q^{(d)}_{t|0}\qty(y^{(d)}\middle|x^{(d)}_0)}{q^{(d)}_{t|0}\qty(x^{(d)}\middle|x^{(d)}_0)}q^{(d)}_{0|t}\qty(x^{(d)}_0\middle| x^{(1:D)}),\\
...
\begin{algorithm}
  ...
  \label{TrainAlg}
  \begin{algorithmic}
    \Require $\mathbf{m}$, $q_0\qty(x^{(1:D)})$, $\lambda_1$, $\lambda_2$, $\lambda_3$, $T$, $0<t_1<t_2<t_3<\cdots<t_n=T$.
    \Repeat
    ...
    \State
    \begin{eqnarray*}
      &&q^{(d)}_{t|0}\qty(x^{(d)}_t\middle|\cdot) \gets \alpha^{(d)}_{t|0} \mathbf{x}^{(d)}_t + \qty(1 - \alpha^{(d)}_{t|0})\left\langle\mathbf{x}^{(d)}_t, \mathbf{m}_d\right\rangle\mathbf{1},\\
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \State Draw $x^{(d)}_{t_{i-1}}\sim \bar{p}^{\theta,(d)}_{\Delta t}\qty(\cdot|x^{(1:D)}_{t_{i-1}}) + \qty(1-\sum_{y}\bar{p}^{\theta,(d)}_{\Delta t}\qty(y|x^{(1:D)}_{t_{i-1}}))\mathbf{x}^{(d)}_{t_{i-1}}$.
    \EndFor
    \EndIf
    \EndFor
  \end{algorithmic}
