...
import torch
from loss_function import cross_entropy_reg
...
def train_loop():
    ...
    accelerator = Accelerator(
        ...
        project_dir=(args.data_file.parent / "Lindel" /"output" / "tensorboard" / ymd).as_posix()
    )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                if args.target_model == "indel":
                    logits = model(batch['input_indel'])
                    counts = batch['del_ins_count']
                elif args.target_model == "ins":
                    logits = model(batch['input_ins'])
                    counts = batch['ins_count']
                elif args.target_model == "del":
                    logits = model(batch['input_del'])
                    counts = batch['del_count']
                loss = cross_entropy_reg(logits, counts, model.linear.weight)
        ...
        if accelerator.is_main_process:
            ...
            with torch.no_grad():
                ...
                for step, batch in enumerate(valid_dataloader):
                    ...
                    if args.target_model == "indel":
                        logits = unwrap_model(batch['input_indel'])
                        counts = batch['del_ins_count']
                    elif args.target_model == "ins":
                        logits = unwrap_model(batch['input_ins'])
                        counts = batch['ins_count']
                    elif args.target_model == "del":
                        logits = unwrap_model(batch['input_del'])
                        counts = batch['del_count']
                    this_valid_loss = cross_entropy_reg(logits, counts, unwrap_model.linear.weight)
                    assert not torch.isnan(this_valid_loss), "the validation loss is nan"
                    valid_loss += this_valid_loss
                    valid_loss_count += logits.shape[0]
