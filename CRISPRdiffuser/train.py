...
def train_loop():
    ...
    if accelerator.is_main_process:
        ...
        #     "total batch per epoch": args.batch_num,
    ...
    for epoch in range(args.epoch_num):
        progress_bar = tqdm(total=min(len(train_dataloader), args.batch_num), disable=not accelerator.is_local_main_process)
        progress_bar.set_description(f"Epoch {epoch}")
        for step, batch in enumerate(train_dataloader):
            # set the limit of batch number
            if step >= args.batch_num:
                break
            ...
            with accelerator.accumulate(model):
                ...
                assert torch.isnan(loss), "the loss function is nan"
        ...
        if accelerator.is_main_process:
            ...
            for valid_epoch in range(args.valid_epoch_num):
                valid_progress_bar = tqdm(total=min(len(valid_dataloader), args.valid_batch_num), disable=not accelerator.is_local_main_process)
                valid_progress_bar.set_description(f"Valid_Epoch {valid_epoch}")
                for valid_step, batch in enumerate(valid_dataloader):
                    # set the limit of validation batch number
                    if valid_step >= args.valid_batch_num:
                        break
                    # optionally save the inference process as images
                    do_save = ((epoch + 1) % args.save_image_epochs == 0 or epoch == args.epoch_num - 1) and (valid_epoch % args.save_image_valid_epochs == 0) and (valid_step % args.save_image_valid_batchs == 0)
                    if do_save:
                        for in_batch in range(0, batch['observation'].shape[0], args.save_image_valid_in_batchs):
                            save_heatmap(batch['observation'][in_batch] / batch['observation'][in_batch].sum(), args.data_file.parent / "output" / "images" / ymd / f"epoch{epoch}" / f"valid_epoch{valid_epoch}" / f"valid_batch{valid_step}" / f"ob{in_batch}.png")
                    save_dir = args.data_file.parent / "output" / "images" if do_save else None
                    # cross entropy
                    this_valid_cross_entropy = inference_function(accelerator.unwrap_model(model), valid_step, batch, valid_epoch, epoch, ymd=ymd, save_dir=save_dir)
                    assert torch.isnan(this_valid_cross_entropy), "the validation cross entropy is nan"
                    valid_cross_entropy += this_valid_cross_entropy
                    valid_cross_entropy_count += batch['condition'].shape[0]

                    valid_progress_bar.update(1)
                    logs = {"cross entropy": this_valid_cross_entropy.item(), "step": valid_global_step}
