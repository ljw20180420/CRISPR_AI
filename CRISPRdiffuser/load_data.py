...
from .config import args, alphacode, ref1len, ref2len, diag_indices, logger
...
def cut2one_hot(cut1, cut2):
    # OUTPUT: one hot tensor dtype=int64 shape=(ref2len+1, ref1len+1, 1)
    # The layer representing 2D cut point
    cut_layer = torch.zeros(ref2len + 1, ref1len + 1, 1, dtype=torch.int64)
    cut_layer[cut2, cut2, 0] = 1
    return cut_layer
...
def num2micro_homology(ref1num, ref2num, cut1, cut2):
    ...
    mh_matrix = F.pad((ref1num[:cut1].view(1, -1) == ref2num[cut2:].view(-1, 1)).to(torch.int64), pad=(0,ref2len-cut1+1,cut2,1), value=0)
    rep_num = torch.cat((
        torch.tensor([-1]),
...
def collect_model_input(examples):
    ...
    return {
        "condition": torch.stack([
            torch.cat((
                num2micro_homology(ref1nums[i], ref2nums[i]).view(len(ref2nums[i])+1, len(ref1nums[i])+1, 1).clamp(0, args.max_micro_homology) / args.max_micro_homology,
                cut2one_hot(examples[i]['cut1'], examples[i]['cut2']),
                kmer2one_hot1D(num2kmer(ref1nums[i]), num2kmer(ref2nums[i])) if not args.cross_reference else kmer2one_hot1D(num2kmer(ref1nums[i]), num2kmer(ref2nums[i]))
            ), dim=2)
            for i in range(len(examples))
        ]),
        "ref1_end": torch.stack([
            torch.tensor(example['ref1_end'])
            for example in examples
        ]),
        "ref2_start": torch.stack([
            torch.tensor(example['ref2_start'])
            for example in examples
        ])
    }

def duplicates_sampler(counts):
    # ex: counts = [10, 12, 9] means there are 10 zeros, 12 ones, 0 twos
    # return a generator samples from above
...
for example in ds['train']:
    try:
        ...
        raise
ds = ds.map(unlist_cuts, batched=True)
...
ds = ds.map(unlist_ref1_end_ref2_start_count, batched=True)
np_gen = np.random.default_rng(args.seed)


train_dataloader = DataLoader(
    ds["train"], batch_sampler=duplicates_sampler(ds["train"]["count"]), collate_fn=collect_model_input
)
epoch_training_steps = (sum(ds["train"]["count"]) - 1) // args.batch_size + 1
valid_dataloader = DataLoader(
    ds["valid"], batch_sampler=duplicates_sampler(ds["valid"]["count"]), collate_fn=collect_model_input
)