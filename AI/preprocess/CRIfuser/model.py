...
class CRIfuserConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        loss_weights: Optional[
            dict[
                Literal[
                    ...
                    "forward_sample_negative_ELBO",
                    "reverse_sample_negative_ELBO",
                    "sample_CE",
                    "non_sample_CE",
                ],
                float,
        ...
        count_normalize: float = 1000.0,
        channels: List = [11, 32, 64, 96, 64, 32, 1],
        MCMC_corrector_factor: List = [1.0, 0.0, 0.001],
        ref1len: int = 127,
        ref2len: int = 127,
...
class CRISPRDiffuserModel(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.register_buffer(
            "stationary_sampler1_probs",
            F.normalize(torch.ones(config.ref1len + 1), p=1.0, dim=0),
        )
        self.register_buffer(
            "stationary_sampler2_probs",
            F.normalize(torch.ones(config.ref2len + 1), p=1.0, dim=0),
        )
        # time
        self.time_emb = nn.Sequential(
            nn.Linear(
                in_features=self.config.channels[1],
                out_features=4 * self.config.channels[1],
            ),
            nn.SiLU(),
            nn.Linear(
                in_features=4 * self.config.channels[1],
                out_features=4 * self.config.channels[1],
            ),
        )
        # down blocks
        ...
        for i in range((len(self.config.channels) - 1) // 2 - 1):
            ...
            self.down_time_embs.append(
                nn.Sequential(
                    nn.Linear(
                        in_features=4 * self.config.channels[1],
                        out_features=self.config.channels[i + 1],
                    ),
                    nn.SiLU(),
                )
            )
            self.down_samples.append(
                nn.MaxPool2d(
                    kernel_size=2
                )  # nn.AvgPool2d(kernel_size=2), nn.Conv2d(channels[i + 1], channels[i + 1], kernel_size=2, stride=2)
            )
        # mid block
        ...
        for i in range(
            (len(self.config.channels) - 1) // 2, len(self.config.channels) - 2
        ):
            ...
            self.up_first_convs.append(
                nn.Sequential(
                    nn.Conv2d(
                        in_channels=self.config.channels[i + 1]
                        + self.config.channels[len(self.config.channels) - i - 2],
                        out_channels=self.config.channels[i + 1],
                        kernel_size=3,
                        padding=1,
                    ),
    ...
    def forward(
        ...
        x = torch.cat(
            (
                (
                    F.one_hot(
                        x1t, num_classes=len(self.stationary_sampler1_probs)
                    ).view(batch_size, 1, -1)
                    * F.one_hot(
                        x2t, num_classes=len(self.stationary_sampler2_probs)
                    ).view(batch_size, -1, 1)
                )[:, None, :, :],
                condition,
            ),
        ...
        t_emb = get_timestep_embedding(
            t,
            embedding_dim=self.config.channels[1],
        ...
        for i in range(len(self.down_first_convs)):
            down_xs.append(
                self.down_second_convs[i](
                    ...
                    + self.down_time_embs[i](t_emb)[:, :, None, None]
                )
            )
            x = self.down_samples[i](down_xs[-1])
        x = self.mid_second_conv(
            self.mid_first_conv(x) + self.mid_time_emb(t_emb)[:, :, None, None]
        )
        for i in range(len(self.up_first_convs)):
            x = self.up_second_convs[i](
                ...
                + self.up_time_embs[i](t_emb)[:, :, None, None]
            )
        p_theta_0_logit = self.out_cov(x)
        if observation is not None:
            self.loss = self.continuous_time_loss_function(
                x1t, x2t, t, p_theta_0_logit, observation
            )
            return {"p_theta_0_logit": p_theta_0_logit, "loss": self.loss}
        return {"p_theta_0_logit": p_theta_0_logit}

    def continuous_time_loss_function(
        self,
        x1t: torch.Tensor,
        x2t: torch.Tensor,
        t: torch.Tensor,
        p_theta_0_logit: torch.Tensor,
        observation: torch.Tensor,
    ):
        def get_q_rkm_d(stationary_sampler_probs, xt):
            xt_one_hot = F.one_hot(xt, len(stationary_sampler_probs))
            q_rkm_d = (
                alpha_t[:, None] * xt_one_hot
                + ((1 - alpha_t) * stationary_sampler_probs[xt])[:, None]
            )
            return q_rkm_d
        ...
        p_theta_0 = F.softmax(p_theta_0_logit.view(batch_size, -1), dim=1).view(
            batch_size,
            len(self.stationary_sampler2_probs),
            len(self.stationary_sampler1_probs),
        )
        log_p_theta_0 = F.log_softmax(p_theta_0_logit.view(batch_size, -1), dim=1).view(
            batch_size,
            len(self.stationary_sampler2_probs),
            len(self.stationary_sampler1_probs),
        )
        ...
        q_0_give_t = F.normalize(
            (observation * q_rkm_1[:, None, :] * q_rkm_2[:, :, None]).view(
                batch_size, -1
            ),
            p=1.0,
            dim=1,
        ...
        common_negative_ELBO = self.stationary_sampler1_probs[x1t] * g_theta_1_t.sum(
            dim=1
        ...
        reverse_negative_ELBO = common_negative_ELBO - (
            (g_1_t * log_g_theta_1_t).sum(dim=1) + (g_2_t * log_g_theta_2_t).sum(dim=1)
        )

        MCMC_corrector = -(
            log_p_theta_0.view(batch_size, -1) * q_0_give_t.view(batch_size, -1)
        ).sum(dim=1)

        return (
            observation.sum(dim=(1, 2))
            / self.config.count_normalize
            * (
