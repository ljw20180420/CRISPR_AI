...
from transformers import PreTrainedModel, PretrainedConfig, __version__
...
class Train:
    ...
    def load_checkpoint(self, checkpoint_path: str, model: PreTrainedModel):
        ...
        self.logger.info(f"Loading trainer state from {checkpoint_path}.")
        with open(os.path.join(checkpoint_path, "trainer_state.json"), "r") as fd:
            trainer_state = json.load(fd)
        for param, value in trainer_state:
            assert hasattr(self, param), f"Train does not have parameter {param}"
